 ...
 *
 *
 * $Id: i2c_template.c 523 2013-11-13 12:36:08Z zajost $
 *
 *
 * \par Description:
 ...
 *--------------------------------------------------------------------
 */
#include "platform.h"	// platform must be included before i2c
#include "i2c.h"
...
void i2cReceiveData(uint8 *buf, uint8 length)
{
  ...
  if(length > 1)
  {      
    IICC_TXAK = 0;                          // Ab jetzt für empfangene Bytes Ack's erzeugen
...
void i2cInit()
{
	// @ToDo enable I2C with 400 kHz SCL clock frequency
	
	// init IICF register (speed control for I2C module)
	IICF_MULT = 0x01;		// set multiplication factor to 2
	IICF_ICR = 0x05;		// set the divisor factor to 30
	
	// init IICC register (main config for I2C module)
	IICC_IICEN = 1;			// enable the I2C module
}
